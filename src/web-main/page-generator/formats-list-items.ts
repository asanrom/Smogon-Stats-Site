 ...
 *
 * Formats list for items usage stats.
 */
...
export class FormatsListItemsPG implements IPageGenerator {
    ...
    public generateHTML(data: IGenerationData, language: Language, print: PrintFunction,
        ...
        print("<span class=\"mdl-radio__label\">" + language.getText("flist.items.sortabc") + "</span>");
        ...
        print("<span class=\"mdl-radio__label\">" + language.getText("flist.items.sorttotal") + "</span>");
        print("</label></div>");
        ...
        print("<span class=\"mdl-checkbox__label\">" + language.getText("flist.items.onlytop")
            + " (&gt; 1500)</span>");
        print("</label></div>");
        ...
        if (data.statsData.formatsItems) {
            const formats = data.statsData.formatsItems.formats;
            for (const format of formats) {
                print("<div class=\"format-card mdl-card mdl-shadow--2dp\" "
                    + "id=\"" + format.id + "-" + format.baseline + "-" + format.totalItems
                    + "\" name=\"" + format.id + "-" + format.baseline + "-" + format.totalItems
                    + "\">");
                print("<div class=\"mdl-card__title\">");
                print("<div class=\"format-card-sprite\" style=\""
                    + "background: url('" + Sprites.getItemSpriteURL(format.topItem)
                ...
                print("" + language.getText("flist.items.baseline") + ": " + format.baseline);
                print(", " + language.getText("flist.items.total") + ": " + format.totalItems);
                print(", " + language.getText("flist.items.top") + ": "
                    + escapeHTML(getItemName(format.topItem) || "(none)"));
                print("</div>");
                ...
                print(language.getText("flist.items.view"));
                print("</a></div>");
                print("</div>");
            }
        }
